...
module.exports = {
  SearchAppointment: async (req, res) => {
    try {
      ...
      const { token, state, city, department, Date_of_Appointment } = req.body;
            ...
            if (result) {
              for (var i of result) {
                ...
                var available = await AppointmentSchema.find({
                  DoctorEmail: i.Email,Date:Date_of_Appointment
                }).lean();
  ...
  BookAppointment: async (req, res) => {
    const { ID, token, Day_Time, Date_of_Appointment ,City_of_Appointment} = req.body;
    try {
      if (ID && token && Day_Time && Date_of_Appointment && City_of_Appointment) {
